...
<template>
  <div class="details-face">
    ...
    <h4>CAST :</h4>
    <span v-for="(element, index) in info" :key="'a' + index">
      {{ element.name }}
    </span>

    <h4>GENERI :</h4>
    <span v-for="(genre, index) in genresInfo" :key="'b' + index">
      {{ genre.name }}
    </span>
  </div>
</template>
  ...
  methods: {
    ...
    async getInfo() {
      let response = await this.makeAxiosCall(
        `https://api.themoviedb.org/3/${this.input}/${this.id}/credits`
      );
      let cast = response.data.cast;
      for (let i = 0; i < 5 && i < cast.length; i++) this.info.push(cast[i]);
    },
    async getGenres() {
      let response = await this.makeAxiosCall(
        `https://api.themoviedb.org/3/${this.input}/${this.id}`
      );
